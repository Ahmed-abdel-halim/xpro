...
<div class="flex justify-center items-center min-h-[70vh]">
    <div class="bg-white dark:bg-white/5 p-10 rounded-[2.5rem] w-full max-w-md shadow-2xl shadow-gray-200/50 dark:shadow-none border border-gray-100 dark:border-white/10 relative overflow-hidden">
        ...
        <h2 class="text-3xl font-black mb-8 text-center text-[var(--text-color)] dark:text-transparent dark:bg-clip-text dark:bg-gradient-to-l dark:from-sky-400 dark:to-emerald-400 relative z-10">أهلاً بك مجدداً</h2>
            ...
            <div class="space-y-6 relative z-10">
                ...
                <div class="flex items-center justify-between text-sm">
                    <label class="flex items-center space-x-2 space-x-reverse cursor-pointer group">
                        <input type="checkbox" name="remember" class="w-4 h-4 rounded bg-gray-50 border-gray-300 text-amber-500 focus:ring-amber-500 dark:bg-white/5 dark:border-white/10 dark:text-sky-500 dark:focus:ring-sky-500 transition">
                        <span class="text-gray-600 dark:text-gray-400 group-hover:text-amber-600 dark:group-hover:text-sky-400 font-medium transition">تذكرني</span>
                    </label>
                    @if (Route::has('password.request'))
                        <a href="{{ route('password.request') }}" class="text-amber-500 hover:text-amber-600 dark:text-sky-400 dark:hover:text-sky-300 font-bold transition">نسيت كلمة السر؟</a>
                    @endif
                </div>
                ...
                <p class="text-center text-gray-600 dark:text-gray-400 font-medium mt-6 pt-6 border-t border-gray-100 dark:border-white/10">
                    ليس لديك حساب؟ <a href="{{ route('register') }}" class="text-amber-500 hover:text-amber-600 dark:text-sky-400 dark:hover:text-sky-300 font-bold transition mr-1">إنشاء حساب جديد</a>
                </p>
            </div>
        </form>
    </div>
