        ...
        <div class="bg-white dark:bg-[#0f172a] p-8 rounded-3xl shadow-xl border border-gray-100 dark:border-white/10">
            ...
            @if(isset($pendingPayment))
                <div class="mb-6 p-4 bg-amber-500/10 border border-amber-500/20 rounded-2xl flex items-center gap-4 text-amber-600 dark:text-amber-400">
                    ...
                    <p class="text-sm font-bold">لديك عملية دفع سابقة بانتظار التأكيد لهذا الكورس. إذا كنت قد دفعت بالفعل، يرجى الانتظار قليلاً أو التواصل مع الدعم.</p>
                </div>
            @endif
            ...
            <form action="{{ route('payment.process', $course->id) }}" method="POST" class="space-y-4">
                    ...
                    <div class="p-6 rounded-2xl border-2 peer-checked:border-amber-500 dark:peer-checked:border-sky-500 border-gray-100 dark:border-white/5 bg-gray-50 dark:bg-white/5 group-hover:bg-gray-100 dark:group-hover:bg-white/10 transition flex items-center justify-between shadow-sm dark:shadow-none">
                        <div class="flex items-center space-x-4 space-x-reverse">
                            ...
                            <div>
                                ...
                                <div class="text-xs text-gray-500 font-medium mt-1">فيزا، ماستر كارد (Visa / Mastercard)</div>
                            </div>
                        </div>
                ...
                {{-- زين كاش --}}
                <label class="block cursor-pointer group">
                    <input type="radio" name="method" value="zaincash" class="hidden peer">
                    <div class="p-6 rounded-2xl border-2 peer-checked:border-emerald-500 border-gray-100 dark:border-white/5 bg-gray-50 dark:bg-white/5 group-hover:bg-gray-100 dark:group-hover:bg-white/10 transition flex items-center justify-between shadow-sm dark:shadow-none">
                        <div class="flex items-center space-x-4 space-x-reverse">
                            ...
                            <div>
                                <div class="font-bold text-[var(--text-color)] dark:text-white">زين كاش</div>
                                <div class="text-xs text-gray-500 font-medium mt-1">الدفع عبر محفظة زين كاش الإلكترونية (ZainCash)</div>
                            </div>
                        </div>
                ...
                <div id="zaincash-phone-container" class="hidden mt-4 p-4 bg-emerald-500/5 border border-emerald-500/10 rounded-2xl">
                    <label class="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-3">رقم الهاتف المسجل عليه زين كاش</label>
                    <div class="relative">
                        <input type="text" name="wallet_phone" id="wallet_phone" placeholder="مثلاً: 07801234567" 
                    ...
                    <p class="text-[11px] text-emerald-600 dark:text-emerald-400 mt-2 font-bold">
                        ...
                        يرجى كتابة رقم الهاتف المسجل عليه محفظة زين كاش.
                    </p>
                </div>
                ...
                <div class="pt-8">
                    ...
                    <p class="text-center text-gray-500 text-xs mt-4 font-bold">
                        بضغطك على الزر، سيتم تحويلك إلى صفحة الدفع الآمنة
                    </p>
                </div>
            </form>
        </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        const zaincashContainer = document.getElementById('zaincash-phone-container');
        const walletPhoneInput = document.getElementById('wallet_phone');

        function toggleZaincash() {
            const selectedMethod = document.querySelector('input[name="method"]:checked').value;
            if (selectedMethod === 'zaincash') {
                zaincashContainer.classList.remove('hidden');
                walletPhoneInput.setAttribute('required', 'required');
            } else {
                zaincashContainer.classList.add('hidden');
                walletPhoneInput.removeAttribute('required');
            }
        }

        methodInputs.forEach(input => {
            input.addEventListener('change', toggleZaincash);
        });
